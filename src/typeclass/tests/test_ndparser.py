...
class NDParserTestCase(unittest.TestCase):
    def assert_ndparser_expr_equal(self, lhs, rhs):
        lhs_parser = run(lhs, None, None).force()
        rhs_parser = run(rhs, None, None).force()

        for s in fx_ndparser.inputs():
            with self.subTest(input=s):
                self.assertEqual(lhs_parser.run(s), rhs_parser.run(s))


class TestNDParserFunctor(NDParserTestCase):
    def test_functor_identity(self):
        for value in fx_ndparser.values():
            with self.subTest(value=value):
                lhs, rhs = functor_identity_expr(value)
                self.assert_ndparser_expr_equal(lhs, rhs)
...
class TestNDParserApplicative(NDParserTestCase):
    def test_applicative_identity(self):
        for value in fx_ndparser.values():
            with self.subTest(value=value):
                lhs, rhs = applicative_identity_expr(NDParser, value)
                self.assert_ndparser_expr_equal(lhs, rhs)
        ...
        for f in funcs:
            for x in xs:
                with self.subTest(f=f, x=x):
                    lhs, rhs = applicative_homomorphism_expr(NDParser, f, x)
                    self.assert_ndparser_expr_equal(lhs, rhs)

    def test_applicative_interchange(self):
        function_values = [
            NDParser(lambda s: [(lambda x: x + 1, s)]),
            NDParser(lambda s: [(lambda x: x * 2, s)]),
            NDParser(lambda s: [(lambda x: x + 1, s), (lambda x: x * 2, s)]),
        ]
        ys = [0, 1, 10]
        ...
        for u in function_values:
            for y in ys:
                with self.subTest(u=u, y=y):
                    lhs, rhs = applicative_interchange_expr(NDParser, u, y)
                    self.assert_ndparser_expr_equal(lhs, rhs)

    def test_applicative_composition(self):
        us = [
            NDParser(lambda s: [(lambda x: x + "!", s)]),
            NDParser(lambda s: [(lambda x: f"[{x}]", s)]),
            NDParser(lambda s: [(lambda x: x + "!", s), (lambda x: f"[{x}]", s)]),
        ]
        vs = [
            NDParser(lambda s: [(lambda x: x.upper(), s)]),
            NDParser(lambda s: [(lambda x: x + x, s)]),
            NDParser(lambda s: [(lambda x: x.upper(), s), (lambda x: x + x, s)]),
        ]

        for u in us:
            for v in vs:
                for w in fx_ndparser.values():
                    with self.subTest(u=u, v=v, w=w):
                        lhs, rhs = applicative_composition_expr(NDParser, u, v, w)
                        self.assert_ndparser_expr_equal(lhs, rhs)

    def test_applicative_then_definition(self):
        for fa in fx_ndparser.values():
            for fb in fx_ndparser.values():
...
class TestNDParserMonad(NDParserTestCase):
    def test_monad_left_identity(self):
        xs = [0, 1, 10, "x"]
        funcs = [
            lambda x: NDParser(lambda s: [(x, s)]),
            lambda x: NDParser(lambda s: [(f"{x}!", s)]),
            lambda x: NDParser(lambda s: [(x, s), (f"{x}!", s)]),
        ]

        for x in xs:
            for f in funcs:
                with self.subTest(x=x, f=f):
                    lhs, rhs = monad_left_identity_expr(NDParser, x, f)
                    self.assert_ndparser_expr_equal(lhs, rhs)

    def test_monad_right_identity(self):
        for value in fx_ndparser.values():
            with self.subTest(value=value):
                lhs, rhs = monad_right_identity_expr(value, NDParser)
                self.assert_ndparser_expr_equal(lhs, rhs)

    def test_monad_associativity(self):
        f = lambda x: NDParser(lambda s: [(f"{x}!", s), (f"[{x}]", s)])
        g = lambda x: NDParser(lambda s: [(f"<{x}>", s)])
    ...
    def test_monad_join_definition(self):
        values = [
            NDParser(lambda s: []),
            NDParser(lambda s: [(NDParser(lambda t: [("x", t)]), s)]),
            NDParser(lambda s: [(NDParser(lambda t: [(t, "")]), s)]),
            NDParser(
                lambda s: [
                    (NDParser(lambda t: [("x", t)]), s),
                    (NDParser(lambda t: [("y", t)]), s),
                ]
            ),
        ]
    ...
    def test_monad_mthen_definition(self):
        for ma in fx_ndparser.values():
            for mb in fx_ndparser.values():
...
class TestNDParserAlternative(NDParserTestCase):
    def test_alternative_left_identity(self):
        for value in fx_ndparser.values():
            with self.subTest(value=value):
                lhs, rhs = alternative_left_identity_expr(NDParser, value)
                self.assert_ndparser_expr_equal(lhs, rhs)

    def test_alternative_right_identity(self):
        for value in fx_ndparser.values():
            with self.subTest(value=value):
                lhs, rhs = alternative_right_identity_expr(NDParser, value)
                self.assert_ndparser_expr_equal(lhs, rhs)

    def test_alternative_associativity(self):
        for x in fx_ndparser.values():
            for y in fx_ndparser.values():
                for z in fx_ndparser.values():
                    with self.subTest(x=x, y=y, z=z):
                        lhs, rhs = alternative_associativity_expr(x, y, z)
                        self.assert_ndparser_expr_equal(lhs, rhs)

    def test_alternative_some_definition(self):
        for value in fx_ndparser.consuming_values():
            with self.subTest(value=value):
                lhs, rhs = alternative_some_expr(NDParser, value)
                self.assert_ndparser_expr_equal(lhs, rhs)

    def test_alternative_many_definition(self):
        for value in fx_ndparser.consuming_values():
            with self.subTest(value=value):
                lhs, rhs = alternative_many_expr(NDParser, value)
                self.assert_ndparser_expr_equal(lhs, rhs)
