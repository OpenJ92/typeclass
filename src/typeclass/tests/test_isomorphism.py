...
from typeclass.tests.fixtures import isomorphism as fx_isomorphism
...
class IsomorphismTestCase(unittest.TestCase):
    def assert_isomorphism_expr_equal(self, lhs, rhs, inputs=None):
        left = run(lhs, None, None).force()
        right = run(rhs, None, None).force()

        if inputs is None:
            inputs = fx_isomorphism.inputs()
...
class TestIsomorphismSemigroupoid(IsomorphismTestCase):
    def test_semigroupoid_associativity(self):
        for f, g, h in fx_isomorphism.triples():
            with self.subTest(f=f, g=g, h=h):
                lhs, rhs = semigroupoid_associativity_expr(f, g, h)
                self.assert_isomorphism_expr_equal(lhs, rhs)
    ...
    def test_semigroupoid_compose_rcompose(self):
        for f, g in fx_isomorphism.pairs():
            with self.subTest(f=f, g=g):
                lhs, rhs = semigroupoid_compose_rcompose_expr(f, g)
                self.assert_isomorphism_expr_equal(lhs, rhs)


class TestIsomorphismCategory(IsomorphismTestCase):
    def test_category_left_identity(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = category_left_identity_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs)

    def test_category_right_identity(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = category_right_identity_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs)

    def test_category_left_ridentity(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = category_left_ridentity_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs)

    def test_category_right_ridentity(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = category_right_ridentity_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs)


class TestIsomorphismGroupoid(IsomorphismTestCase):
    def test_groupoid_left_invert(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = groupoid_left_invert_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs)

    def test_groupoid_right_invert(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = groupoid_right_invert_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs)


class TestIsomorphismArrow(IsomorphismTestCase):
    def test_arrow_identity(self):
        lhs, rhs = arrow_identity_expr(Isomorphism)
        self.assert_isomorphism_expr_equal(lhs, rhs)

    def test_arrow_composition(self):
        for f, g in fx_isomorphism.composition_pairs():
            with self.subTest(f=f, g=g):
                lhs, rhs = arrow_composition_expr(Isomorphism, f, g)
                self.assert_isomorphism_expr_equal(lhs, rhs)

    def test_arrow_first_identity(self):
        lhs, rhs = arrow_first_identity_expr(Isomorphism)
        self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.pair_inputs())

    def test_arrow_first_composition(self):
        for f, g in fx_isomorphism.pairs():
            with self.subTest(f=f, g=g):
                lhs, rhs = arrow_first_composition_expr(Isomorphism, f, g)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.pair_inputs())

    def test_arrow_second(self):
        for value in fx_isomorphism.values():
            with self.subTest(value=value):
                lhs, rhs = arrow_second_expr(Isomorphism, value)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.pair_inputs())

    def test_arrow_split(self):
        for value1, value2 in fx_isomorphism.pairs():
            with self.subTest(value1=value1, value2=value2):
                lhs, rhs = arrow_split_expr(Isomorphism, value1, value2)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.pair_inputs())

    def test_arrow_fanout(self):
        for value1, value2 in fx_isomorphism.pairs():
            with self.subTest(value1=value1, value2=value2):
                lhs, rhs = arrow_fanout_expr(Isomorphism, value1, value2)
                self.assert_isomorphism_expr_equal(lhs, rhs)


class TestIsomorphismArrowChoice(IsomorphismTestCase):
    def test_arrowchoice_left_naturality(self):
        for f in fx_isomorphism.arrow_functions():
            with self.subTest(f=f):
                lhs, rhs = arrowchoice_left_naturality_expr(Isomorphism, f)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_right_naturality(self):
        for f in fx_isomorphism.arrow_functions():
            with self.subTest(f=f):
                lhs, rhs = arrowchoice_right_naturality_expr(Isomorphism, f)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_left_identity(self):
        lhs, rhs = arrowchoice_left_identity_expr(Isomorphism)
        self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_right_identity(self):
        lhs, rhs = arrowchoice_right_identity_expr(Isomorphism)
        self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_left_composition(self):
        for f, g in fx_isomorphism.pairs():
            with self.subTest(f=f, g=g):
                lhs, rhs = arrowchoice_left_composition_expr(Isomorphism, f, g)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_right_composition(self):
        for f, g in fx_isomorphism.pairs():
            with self.subTest(f=f, g=g):
                lhs, rhs = arrowchoice_right_composition_expr(Isomorphism, f, g)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_split_choice(self):
        for value1, value2 in fx_isomorphism.pairs():
            with self.subTest(value1=value1, value2=value2):
                lhs, rhs = arrowchoice_split_choice_expr(Isomorphism, value1, value2)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())

    def test_arrowchoice_fanin(self):
        for value1, value2 in fx_isomorphism.pairs():
            with self.subTest(value1=value1, value2=value2):
                lhs, rhs = arrowchoice_fanin_expr(Isomorphism, value1, value2)
                self.assert_isomorphism_expr_equal(lhs, rhs, inputs=fx_isomorphism.either_inputs())


class TestIsomorphismArrowApply(IsomorphismTestCase):
    def test_arrowapply_arr_app(self):
        for f in fx_isomorphism.arrow_functions():
            with self.subTest(f=f):
                lhs, rhs = arrowapply_arr_app_expr(Isomorphism, f)
                self.assert_isomorphism_expr_equal(lhs, rhs)
