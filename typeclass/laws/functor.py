...
def assert_functor_identity(functor: Functor[A] & Eq):
    assert functor.fmap(lambda x: x) == functor, "Functor identity law failed"

def assert_functor_composition(functor: Functor[A] & Eq, f: Callable[[A], B], g: Callable[[B], C]):
    assert functor.fmap(lambda x: g(f(x))) == functor.fmap(f).fmap(g), "Functor composition law failed"

def assert_functor_laws(functor: Functor[A] & Eq, f: Callable[[A], B], g: Callable[[B], C]):
